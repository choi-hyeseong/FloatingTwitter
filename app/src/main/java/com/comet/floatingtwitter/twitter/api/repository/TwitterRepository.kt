...
interface TwitterRepository {
     ...
     */
    fun startAPIListening(callback : (List<EventData>) -> Unit)

    /**
     * 리스너를 중단합니다.
     */
    fun stopAPIListening()
     ...
     * 짹쨱이의 유저 이미지를 가져옵니다.
     */
    fun getAvatarResource(user : User, token: OAuthToken) : Drawable

    /**
     * 짹짹이의 유저 정보를 가져옵니다.
     * @see TwitterAPIDao.getUserInfo
     */
    fun getUserInfo(token : OAuthToken) : User
}